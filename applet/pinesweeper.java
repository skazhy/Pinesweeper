...
public class pinesweeper extends PApplet {
...
class Field {
  ...
  public void prepareField() {
    ...
    for (int i=0; i< pine_count; i++) {
      while (add_pine != 1) {
        temp_x = PApplet.parseInt(random(16)) + 1;
        temp_y = PApplet.parseInt(random(16)) + 1;
  ...
  public void drawField() {
    ...
    for (int i=0; i<17; i++) {
      for (int j=0; j<17; j++) {
        if (visible[i][j] == 0 && marked[i][j] == 0) fill(0xff45CC33);
        if (visible[i][j] == 1) fill(0xffDBD997);
  ...
  public void markPine(int x, int y) {
    int lock = 0;
    //mark a pine
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#DFDFDF", "pinesweeper" });
  }
}
